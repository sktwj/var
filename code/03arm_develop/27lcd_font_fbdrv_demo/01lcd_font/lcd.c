...
#define WPALCON_H 	(*(volatile u32 *)(LCD_BASE + 0x019c))
...
#define PAL8_SWAP_MODE 0b0100
	u8 swap_mode; //相邻的几个像素的排列顺序,看手册, 如果是RGB565，看P1779,如果是RGB888，P1774,PAL8,P1783
...
static void draw_pixel_pal8(struct win_info *info, ssize_t x, ssize_t y, u32 color);
...
static void draw_pixel_pal8(struct win_info *info, ssize_t x, ssize_t y, u32 pal_index)
{
	*(u8*)(info->start_addr + XSIZE * y * info->pixel_size + x * info->pixel_size) = pal_index;
}
...
//切换显存首末地址
static void change_buf(struct win_info *info, u32 new_addr)
{
	//不能在帧周期中正在传输像素数据的时候修改显存首末地址，需要避开像素传输的时间段
	/* VIDCON1[14:13]: 00-VSYNC, 01-back porch, 10-active, 11-front porch */
	while (((VIDCON1 >> 13) & 3) == 0b10) {}

	switch (info - win) {
	case 0:
		VIDW00ADD0B0 = new_addr;
		VIDW00ADD1B0 = new_addr + XSIZE * YSIZE * win[0].pixel_size;
		break;
	case 1:
		VIDW01ADD0B0 = new_addr;
		VIDW01ADD1B0 = new_addr + XSIZE * YSIZE * win[1].pixel_size;
		break;
	case 2:
		VIDW02ADD0B0 = new_addr;
		VIDW02ADD1B0 = new_addr + XSIZE * YSIZE * win[2].pixel_size;
		break;
	default:
		printf("unsupport window num %d\n", info - win);
		break;
	}
}
...
static void mov_pic()
{
	...
	static u32 cur_addr = 0;
	static s32 offset = 1;

	if (!cur_addr) {
		cur_addr = win[0].start_addr;
	}

	cur_addr += offset * XSIZE * win[0].pixel_size;

	if (cur_addr - win[0].start_addr >= (1433 - YSIZE) * XSIZE * win[0].pixel_size) {
		offset = -1;
	} else if (cur_addr - win[0].start_addr <= 0) {
		offset = 1;
	}

	//printf("offset = %d\n", offset);

	change_buf(&win[0], cur_addr);
}
 ...
 * @x, y: 字符的左上角坐标
 */
static void __draw_ch(struct win_info *info, size_t x, size_t y, u8 ch, u32 color, const struct font_desc *fontdesc)
{
	if (fontdesc->width > 8) {
		return; //FIXME
	}
	...
	//单个字符像素数据占据多少字节
	size_t font_size = (fontdesc->width + 7) / 8 * fontdesc->height;

	//指向目标字符的像素数据的首地址
	u8 *font = ((u8*)fontdesc->data + ch * font_size);

	size_t ix, iy;
	for (iy = 0; iy < fontdesc->height; ++iy) {
		for (ix = 0; ix < fontdesc->width; ++ix) {
			if (font[iy] & (1 << (7 - ix))) {
				info->draw_pixel(info, x + ix, y + iy, color);
			}
		}
...
static void draw_ch(u8 ch)
{
	//光标位置坐标
	static size_t lcd_ch_x, lcd_ch_y;

	//是否换行
	int is_change_line = 0;
	...
	__draw_ch(win + 0, lcd_ch_x, lcd_ch_y, ch, RGB888(255, 255, 255), &font_vga_8x16);

	//光标向右移动一个字符宽度
	lcd_ch_x += font_vga_8x16.width;
	//是否光标已经到了行末
	if (lcd_ch_x >= XSIZE - 1) {
		is_change_line = 1;
	}
...
static void lcd_init()
{
	 ...
	 * [6]：0-平面混合，整个window使用统一的alpha值，整个window将呈现一致的透明度
	 * 1-像素混合，每个像素点可以单独指定alpha值，每个像素都可以呈现不同的透明度
	 * [1]: 如果是平面混合，本位将用于选择2个alpha值（在VIDOSD2C中设置）中的一个	 * 如果是像素混合，那么每个像素都有单独的alpha值，本位为0，那么可以选择1位alpha值的色深，本位为1，则只能选择多位alpha值（共两种，8位alpha和4位alpha值）
	 */

	WINCON2 = win[2].bpp << 2 | win[2].swap_mode << 15;

	//设置两个可选alpha值，可对3原色分别设置不同的透明度
	VIDOSD2C = win[2].alpha_val << 12 | win[2].alpha_val << 16 | win[2].alpha_val << 20 | 10 << 8 | 10 << 4 | 10;

	//指定window显示区域的坐标范围
	VIDOSD2A = 0; //显示区域左上角坐标
	VIDOSD2B = (XSIZE - 1) << 11 | (YSIZE - 1); //右下角坐标
	//指定的显示区域的大小,注!意!单!位!
	VIDOSD2D = XSIZE * YSIZE * win[2].pixel_size / 4;

	//指定显存的首末地址
	//W00表示windows0，ADD0表示起始地址，ADD1表示显存终止地址，B0中的0表示buf0的显存首地址
	VIDW02ADD0B0 = win[2].start_addr;
	//显存终止地址，查看4412手册P1813，如果需要支持横向滚动的话，终止地址中将包含offsize的大小，如果不支持横向滚动，offsize就是0
	VIDW02ADD1B0 = win[2].start_addr + XSIZE * YSIZE * win[2].pixel_size;
	//有关横向移动显示区域相关，OFFSIZE指允许的横向移动范围, PAGEWIDTH指X方向上的字节数（即一行像素所占的字节数）。一般不用支持横向移动。如果不打算支持横向移动，那么OFFSIZE_F = 0
	VIDW02ADD2 = XSIZE * win[2].pixel_size;

	//==================颜色表===================
	//切换颜色表模式为update模式，此模式下我们可以进行颜色表的配置和填充操作
	WPALCON_L |= 1 << 9;

	//将window2的颜色表格式设为888，颜色值高2位在WPALCON_H[10:9]，低一位在WPALCON_L[6]
	WPALCON_L |= 1 << 6;
	WPALCON_H = (WPALCON_H & ~(3 << 9)) | 0b10 << 9;

	//填充颜色表
	WIN2PAL(0) = RGB888(255, 0, 0);
	WIN2PAL(1) = RGB888(0, 255, 0);
	WIN2PAL(2) = RGB888(0, 0, 255);
	WIN2PAL(3) = RGB888(0, 0, 0);
	WIN2PAL(4) = RGB888(255, 255, 255);
	WIN2PAL(5) = RGB888(255, 255, 0);

	//切换颜色表模式为normal模式，完成初始化和填充操作，进入正常使用模式
	WPALCON_L &= ~(1 << 9);
	...
	clear_screen(win + 0, RGB888(0, 0, 0));

#if 0
	clear_screen(win + 1, RGBA888(1, 0, 0, 255));
	draw_rect(win + 1, XSIZE / 4, YSIZE / 4, XSIZE / 2, YSIZE / 2, RGBA888(0, 255, 0, 0));

	clear_screen(win + 2, 5);
	draw_rect(win + 2, XSIZE / 6, YSIZE / 6, XSIZE / 3, YSIZE / 3, 1);
#endif

	//test changebuff
	//wdt_init(10, mov_pic);

	draw_str("test lcd draw font\n");
	draw_str("test change line\n");
}
