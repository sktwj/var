...
static long ldm_ioctl(struct file *fp, unsigned int cmd, unsigned long data)
{
	...
	struct ldm_info *ldm = container_of(fp->f_op, struct ldm_info, ops);
...
static int ldm_probe(struct platform_device *pdev)
{
	...
	struct ldm_info *ldm = (struct ldm_info*)kzalloc(sizeof(struct ldm_info), GFP_KERNEL);
	if (!ldm) {
		printk(KERN_ERR "kzalloc failed\n");
		goto err_kzalloc_ldm;
	}

	//将ldm塞入pdev的私有数据空间，所有函数就可以通过pdev来获取ldm
	...
	//注册中断
	ret = request_irq(ldm->irqno, wdt_handler, IRQF_TRIGGER_NONE, "ldm", (void*)ldm);
	if (ret < 0) {
