...
//getgeo的写法参考内核驱动drivers/block/hd.c中的hd_getgeo函数
...
//===================================================
static int ldm_getgeo(struct block_device *blkdev, struct hd_geometry *geo)
{
	geo->heads = 2;
	geo->sectors = 50;
	geo->cylinders = DEV_SIZE / SECTOR_SIZE / geo->heads / geo->sectors;

	printk("%s: heads=%d, sectors=%d, cylinders=%d\n", __FUNCTION__, geo->heads, geo->sectors, geo->cylinders);
...
static void __exit ldm_exit(void)
{
	...
	del_gendisk(ldm.disk); //add_disk的逆函数
	blk_cleanup_queue(ldm.disk->queue);
	vfree(ldm.addr);
	unregister_blkdev(ldm.disk->major, DEVNAME);
	put_disk(ldm.disk); //alloc_disk的逆函数
}
